<template>
  <v-container>
    ...
    <v-tabs fixed-tabs v-model="filterTab">
      ...
      <v-tab> {{ filterTabItems[3].title }} </v-tab>
    </v-tabs>
    ...
    <v-tabs-items v-model="filterTab">
      ...
      <v-tab-item>
        <v-card flat>
          <v-card-text>거래연도</v-card-text>
          <v-row>
            <v-col class="px-4">
              <v-range-slider v-model="dealyear.range" :min="dealyear.min" :max="dealyear.max">
                <template v-slot:prepend>
                  <v-text-field
                    :value="dealyear.range[0]"
                    class="mt-0 pt-0"
                    hide-details
                    single-line
                    type="number"
  ...
  data() {
    return {
      ...
      filterTabItems: [
        ...
        { title: "면적", value: "area" },
        { title: "준공연도", value: "buildyear" },
      ],
      dealyear: { min: 2010, max: 2022, range: [2010, 2022] },
      area: { min: 0, max: 200, range: [50, 100] },
      price: { min: 0, max: 3000000000, range: [100000000, 1500000000] },
